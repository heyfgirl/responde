      ...
      var executeSql = function() {
        if (self.sql.indexOf('-- ') === 0) {
          ...
          resolve(new Promise(function(resolve, reject) {
            ...
            var afterExecute = function(results) {
              if (benchmark) {
              ...
              if (err) {
                ...
                if (self.sql.indexOf('sqlite_master') !== -1) {
                  ...
                  result = results;//metaData.changes;
            ...
            //run
            self.database.run(self.sql, afterExecute);
            //if (method === 'exec') {
            //  // exec does not support bind parameter
        ...
        if (!tableNames.length) {
          ...
          return Promise.map(tableNames, function(tableName) {
            return new Promise(function(resolve) {
              ...
              self.database.run('PRAGMA table_info(`' + tableName + '`)', function(results) {
                let err = results["error"];
                if (!err) {
