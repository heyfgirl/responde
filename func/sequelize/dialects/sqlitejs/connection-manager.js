...
ConnectionManager = function(dialect, sequelize) {
  ...
  try {
    this.lib = require('sqlite-sync');
  } catch (err) {
    if (err.code === 'MODULE_NOT_FOUND') {
      throw new Error('Please install sqlite-sync && sql.js package manually');
    }
    throw err;
  }
...
ConnectionManager.prototype.getConnection = function(options) {
  ...
  return new Promise(function (resolve, reject) {
    console.log( self.sequelize.options.storage );

    self.connections[options.inMemory || options.uuid] = self.lib.connect( self.sequelize.options.storage);
